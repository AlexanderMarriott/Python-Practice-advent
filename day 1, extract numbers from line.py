...
print(text)'''
#created empty list to store created list items
list1 = []

#turns string into useable list
def create_list():
...
#iterates through and checks for numbers within the list created
import re
numbers_found = []
def find_numbers_in_list(items):
    for item in items:

        numbers_in_item = re.findall(r'\d+', item)
        if numbers_in_item:
            numbers_found.append(numbers_in_item)
    return numbers_found


find_numbers_in_list(list1)

#check function works correctly
print(numbers_found)

#splits numbers that are paired together into single values
split_list = []


for sublist in numbers_found:
...
'''finding the first and last number on each line or duplicating the number if there's only one '''

#empty array to store values
first_last_items = []

#iterates through split_list and appends first and last number in list to first_last_items
for numbers in split_list:
    if len(numbers) == 1: 
        first_last_items.append(int(str(numbers[0]) + str(numbers[0])))
    elif numbers: 
        first_last_items.append(int(str(numbers[0]) + str(numbers[-1])))  # Append 
    else:
        first_last_items.append(0) 


final_sum = sum(first_last_items)
